...
apt-get install -y --force-yes --no-install-recommends apt-transport-https software-properties-common
apt-get update
wget -qO - https://adoptopenjdk.jfrog.io/adoptopenjdk/api/gpg/key/public | apt-key add -
add-apt-repository --yes https://adoptopenjdk.jfrog.io/adoptopenjdk/deb/
apt-get update && apt-get install -y --force-yes --no-install-recommends adoptopenjdk-14-hotspot
